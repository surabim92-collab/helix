...
public class Logincontroller extends BaseController {
    ...
    loginResponse handleLogin(@RequestBody Member member , HttpServletRequest request, HttpServletResponse response) {
       List<Member>x= (List<Member>) GenericDB.getRows(tech.codingclub.helix.tables.Member.MEMBER,Member.class,tech.codingclub.helix.tables.Member.MEMBER.EMAIL.eq(member.getEmail()).and(tech.codingclub.helix.tables.Member.MEMBER.PASSWORD.eq(member.getPassword())),1);
       if(x!=null && !x.isEmpty()){
           //conditioin met
