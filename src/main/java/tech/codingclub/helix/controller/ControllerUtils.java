            ...
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    if (cookie.getName().equals(TOKEN_COOKIE)) {
                        String takenFromCookie = cookie.getValue();
                        Member member = new GenericDB<Member>().getRow(Tables.MEMBER, Member.class, tech.codingclub.helix.tables.Member.MEMBER.TOKEN.eq(takenFromCookie));
